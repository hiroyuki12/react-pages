      ...
      if(user == null) {
        ...
        return (
          <li className="item" key={index}>
            ...
            <a className="QiitaApp-link" href={item.id} target="_blank" rel="noreferrer">{item.title}</a> {moment(item.created).fromNow()} 蝗樒ｭ疲焚:{item.count_reply} PV謨ｰ:{item.count_pv}
          </li>
        );
      }
      else {
        ...
        return (
          <li className="item" key={index}>
            ...
            <a className="QiitaApp-link" href={url} target="_blank" rel="noreferrer">{item.title}</a> {moment(item.created).fromNow()} 蝗樒ｭ疲焚:{item.count_reply} PV謨ｰ:{item.count_pv}
          </li>
        );
      }
    });
